 ...
 * parameter to `next` when it starts (so it is not lost). We also have to introduce an additional
 * parameter to `yieldAll` to start a delegated generator.
 */
...
internal class GeneratorCoroutine<T, R> :
    Generator<T, R>, GeneratorBuilder<T, R>, Continuation<Unit> {
    ...
    override suspend fun yieldAll(generator: Generator<T, R>, param: R) =
        suspendCoroutineUninterceptedOrReturn sc@{ cont ->
            ...
            if (lastValue == null) return@sc Unit // delegated coroutine doesn't generate anything -- resume
            nextStep = { param ->
                lastValue = generator.next(param)
