package ru.aasmc.examples.mutex
...
class Mutex {
            ...
            loop@ while (true) { // lock free loop on state
                ...
                if (curState == -1) {
                    if (state.compareAndSet(-1, 0)) {
                        ...
                        // but mark ourselves as already resumed in queue
                        // (retrieveWaiter will skip marked entries).
                        waiter.resumed = true
                        return@sc Unit // don't suspend, but continue execution with lock
                    }
                } else { // state >= 0 -- already locked --> increase waiters count and
                    // sleep peacefully until resumed
                    check(curState >= 0)
    ...
    fun unlock() {
        while (true) { // lock-free loop on state
            ...
            val curState = state.get()
            if (curState == 0) {
                // cannot have any waiters in this state, because we are holding a mutex and only
                // a mutex-holder can reduce the number of waiters
    ...
    private fun retrieveWaiter(): Waiter? {
        while (true) {
            ...
            // see if this is an _actual_ waiter (not a left-over that had actually
            // acquired the lock in the slow path)
            if (!waiter.resumed) {
                return waiter
            }
